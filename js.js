...
function isLoggedIn(){
	var c = getCookie("log");
	if(c == undefined || c == ""){
		var logstatushtml = "<a href='' onclick='logOut();'>—брос</a>";
		//$('#mfoot').html(logstatushtml);
		var is = false;
	}else{
		var logstatushtml = "<a href='' onclick='logOut();'>—брос</a>";
		//$('#mfoot').html(logstatushtml);
...
function savethis(el){
	...
	if(kc == 67 || kc == 86 || kc == 65 || kc == 32 || kc == 9 || kc == 13 || kc == 16 || kc == 17 || kc == 18 || kc == 19 || kc == 20 || kc == 27 || kc == 33 || kc == 34 || kc == 35 || kc == 36 || kc == 37 || kc == 38 || kc == 39 || kc == 40 || kc == 45 || kc == 46){
		...
		if(isLoggedIn()){
			...
			$("#" + wid).val(getCookie(wid));
		}else{
			var date = new Date(new Date().getTime() + 60*1000*60*72); //60*1000 это 60 секунд
			document.cookie = "log=" + $.md5(Date()) + "; path=/; expires=" + date.toUTCString();
		}
	}
}
